...
struct usb_iddig_info {
	...
	struct mutex iddig_gpio_mode_lock;
	spinlock_t iddig_gpio_mode_slock;
};
...
//	}
#endif
	spin_lock(&info->iddig_gpio_mode_slock);
	if (mtk_idpin_cur_stat == IDPIN_OUT) {
#ifdef CONFIG_OPLUS_CHARGER_MTK6771
		otg_is_exist = 1;
#endif
		mtk_idpin_cur_stat = IDPIN_IN_HOST;
		spin_unlock(&info->iddig_gpio_mode_slock);
		mt_usbhost_connect();
		mt_vbus_on();
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6771
void iddig_gpio_mode_otg_exit(int mode)
{
	struct usb_iddig_info *info = platform_get_drvdata(musb_pltfm_dev);
...
int iddig_gpio_mode(int mode)
{
	...
	if(musb_pltfm_dev != NULL) {
	      iddig_gpio_mode_otg_exit(mode);
	      mutex_lock(&info->iddig_gpio_mode_lock);
	      if(mode == OTGID_GPIO_MODE) {
			...
			if (retval < 0) {
				mutex_unlock(&info->iddig_gpio_mode_lock);
...
static int otg_iddig_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&info->iddig_gpio_mode_slock);
	mutex_init(&info->iddig_gpio_mode_lock);
