...
bool  tp_judge_ic_match_commandline(struct panel_info *panel_data)
{
	...
	case 132780:
		...
		if (strstr(saved_command_line, "nt36525b_hlt")) {
			...
			pr_info("[TP] 206AC tp vendor boe\n");
		}
		break;
	case 20682:
	...
	case 20043:
		...
		is_tp_type_got_in_match = true;
		if (strstr(boot_command_line, "nt36672c_fhdp_dsi_vdo_dphy_jdi_lcm_drv")) {
			tp_used_index = nt36672c;
			g_tp_dev_vendor = TP_JDI;
		}
		if (strstr(boot_command_line, "nt36672c_fhdp_dsi_vdo_dphy_boe_lcm_drv")) {
			tp_used_index = nt36672c_boe;
			g_tp_dev_vendor = TP_BOE;
		}
		if (strstr(boot_command_line, "hx83112f")) {
			tp_used_index = himax_83112f;
			g_tp_dev_vendor = TP_TIANMA;
		}
		if (strstr(boot_command_line, "ili7807s")) {
