...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_info("gc02mo_mipi_mono1 I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
		*sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			#endif
			pr_info("gc02mo_mipi_mono1: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			#ifdef OPLUS_FEATURE_CAMERA_COMMON
		...
		pr_info("gc02mo_mipi_mono1: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
