...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static kal_uint8 gGc2375h_EepData[DUALCAM_CALI_EEPROM_DATA_LENGTH];
static void read_eepromData(void)
{
    kal_uint16 idx = 0;
    kal_uint16 id = 0;
    for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
        gGc2375h_EepData[idx] = read_cmos_eeprom_8(GC2375H_STEREO_START_ADDR + idx);
    }
    for (idx = DUALCAM_CALI_DATA_LENGTH; idx < (CAMERA_MODULE_SN_LENGTH+DUALCAM_CALI_DATA_LENGTH); idx++) {
        gGc2375h_EepData[idx] = read_cmos_eeprom_8((0xE0 + id) & 0xFF);
        LOG_INF("gc2375h_SN[%d]: 0x%x\n", id, gGc2375h_EepData[idx]);
        id++;
    }
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x0);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x1);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x6);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x7);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x8);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0x9);
    gGc2375h_EepData[idx++] = read_cmos_eeprom_8(0xA);
    gGc2375h_EepData[idx] = read_cmos_eeprom_8(0xB);
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				#endif
				pr_info("gc2375h_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				#ifdef OPLUS_FEATURE_CAMERA_COMMON
				read_eepromData();
				#endif
				return ERROR_NONE;
			}
			pr_info("gc2375h_mipi_raw: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
	     ...
	     LOG_INF("[gc2375h] SENSOR_FEATURE_GET_EEPROM_DATA:%d data_lens:%d\n",
					*feature_para_len, DUALCAM_CALI_EEPROM_DATA_LENGTH);
	     memcpy(&feature_para[0], gGc2375h_EepData, DUALCAM_CALI_EEPROM_DATA_LENGTH);
	       break;
	     #endif /*OPLUS_FEATURE_CAMERA_COMMON*/
