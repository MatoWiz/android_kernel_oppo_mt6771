...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static kal_uint8 gHi846_EepData[DUALCAM_CALI_EEPROM_DATA_LENGTH];
static void read_eepromData(void)
{
    kal_uint16 idx = 0;
    kal_uint16 id = 0;
    for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
        gHi846_EepData[idx] = read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537 + idx);
    }
    for (idx = DUALCAM_CALI_DATA_LENGTH; idx < (CAMERA_MODULE_SN_LENGTH+DUALCAM_CALI_DATA_LENGTH); idx++) {
        gHi846_EepData[idx] = read_cmos_eeprom_8((0xB0 + id) & 0xFF);
        LOG_INF("hi846_SN[%d]: 0x%x\n", id, gHi846_EepData[idx]);
        id++;
    }
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x0);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x1);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x6);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x7);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x8);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0x9);
    gHi846_EepData[idx++] = read_cmos_eeprom_8(0xA);
    gHi846_EepData[idx] = read_cmos_eeprom_8(0xB);
}
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					...
					#endif
					pr_info("hi846_mipi_raw2: i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					#ifdef OPLUS_FEATURE_CAMERA_COMMON
					read_eepromData();
					#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
					return ERROR_NONE;
				}
				pr_info("hi846_mipi_raw2: get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: PLATFORM:MT6771,MIPI 4LANE\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			...
			pr_info("hi846_mipi_raw2: open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
        ...
        LOG_INF("[hi846] SENSOR_FEATURE_GET_EEPROM_DATA:%d data_lens:%d\n",
                *feature_para_len, DUALCAM_CALI_EEPROM_DATA_LENGTH);
        memcpy(&feature_para[0], gHi846_EepData, DUALCAM_CALI_EEPROM_DATA_LENGTH);
        break;
        #endif /*OPLUS_FEATURE_CAMERA_COMMON*/
