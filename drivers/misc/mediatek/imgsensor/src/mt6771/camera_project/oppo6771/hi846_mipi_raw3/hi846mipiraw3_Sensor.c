...
static void read_eepromData(void)
{
    ...
    for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
        gHi846_EepData[idx] = read_cmos_eeprom_8(HI846_STEREO_START_ADDR + idx);
    }
			...
			do {
				*sensor_id =return_sensor_id() + 1;
				if (*sensor_id == imgsensor_info.sensor_id) {
					...
					#endif
					pr_info("hi846_mipi_raw3: i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					#ifdef OPLUS_FEATURE_CAMERA_COMMON
					read_eepromData();
					#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
					return ERROR_NONE;
				}
				pr_info("hi846_mipi_raw3: get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("hi846_mipi_raw3: open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
