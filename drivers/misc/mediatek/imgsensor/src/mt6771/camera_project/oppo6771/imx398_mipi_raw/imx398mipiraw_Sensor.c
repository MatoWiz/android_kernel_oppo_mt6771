...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.custom1 = {		/* 30  fps  capture */
		.pclk = 482400000,	/* record different mode's pclk */
		.linelength = 5536,	/* record different mode's linelength */
		.framelength = 3628,	/* record different mode's framelength */
		.startx = 0,	/* record different mode's startx of grabwindow */
		.starty = 0,	/* record different mode's starty of grabwindow */
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static kal_uint8 gImx398_EepData[DUALCAM_CALI_EEPROM_DATA_LENGTH];
static void read_eepromData(void)
{
	kal_uint16 idx = 0;
	kal_uint16 id = 0;
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		gImx398_EepData[idx] = read_cmos_eeprom_8(IMX398_STEREO_START_ADDR + idx);
	}
	for (idx = DUALCAM_CALI_DATA_LENGTH; idx < (CAMERA_MODULE_SN_LENGTH+DUALCAM_CALI_DATA_LENGTH); idx++) {
		gImx398_EepData[idx] = read_cmos_eeprom_8((0xE0 + id) & 0xFF);
		//pr_err("gImx398_EepData[%d]: 0x%x\n", id, gImx398_EepData[idx]);
	    id++;
	}
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x0);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x1);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x6);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x7);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x8);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0x9);
	gImx398_EepData[idx++] = read_cmos_eeprom_8(0xA);
	gImx398_EepData[idx] = read_cmos_eeprom_8(0xB);
}
#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				#endif
				pr_info("imx398_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n",
			...
			pr_info("imx398_mipi_raw: Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable)
	{
		...
		#ifdef OPLUS_FEATURE_CAMERA_COMMON
		LOG_INF("%s.init EMI OPP 0, scenario_id=%d\n", __func__, imgsensor.current_scenario_id);
		if (imgsensor.current_scenario_id == MSDK_SCENARIO_ID_VIDEO_PREVIEW) {
			pm_qos_add_request(&emi_opp_request, PM_QOS_EMI_OPP,  1);
			pm_qos_update_request(&emi_opp_request, 1);
		} else {
			pm_qos_add_request(&emi_opp_request, PM_QOS_EMI_OPP,  0);
			pm_qos_update_request(&emi_opp_request, 0);
		}
		#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
	}
	else
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_EEPROM_DATA:
			pr_err("[imx398] SENSOR_FEATURE_GET_EEPROM_DATA:%d data_lens:%d\n",
				*feature_para_len, DUALCAM_CALI_EEPROM_DATA_LENGTH);
			memcpy(&feature_para[0], gImx398_EepData, DUALCAM_CALI_EEPROM_DATA_LENGTH);
		    break;
		#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
