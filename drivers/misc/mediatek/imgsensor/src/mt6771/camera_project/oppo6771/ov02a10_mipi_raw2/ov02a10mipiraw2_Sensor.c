...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_info("ov02a10_mipi_raw2 I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				#endif
				pr_info("ov02a10_mipi_raw2: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_info("ov02a10_mipi_raw2: Read sensor id fail,write_id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("ov02a10_mipi_raw2: Read sensor id fail, write: 0x%x, sensor: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
