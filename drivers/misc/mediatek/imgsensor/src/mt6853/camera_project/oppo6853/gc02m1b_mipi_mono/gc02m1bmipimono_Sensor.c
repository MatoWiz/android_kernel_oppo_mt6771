    ...
    if(I2C_BUS != 2){
        *sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			#ifdef OPLUS_FEATURE_CAMERA_COMMON
			if(is_project(OPPO_20041) || is_project(OPPO_20042) || is_project(OPPO_20043)){
			    read_eeprom_SN();
			    read_eeprom_CamInfo();
				/*project_20041 = true;
			...
			#endif
			pr_info("gc02m1b_mipi_mono: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;
            	}
		pr_info("gc02m1b_mipi_mono: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
