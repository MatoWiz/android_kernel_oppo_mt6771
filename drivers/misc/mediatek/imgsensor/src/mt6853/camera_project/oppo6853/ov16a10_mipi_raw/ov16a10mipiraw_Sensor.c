        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("ov16a10_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n",
                    imgsensor.i2c_write_id, *sensor_id);
                #ifdef OPLUS_FEATURE_CAMERA_COMMON
                read_EepromQSC();
                read_eeprom_SN();
            ...
            pr_info("ov16a10_mipi_raw: Read sensor id OK:0x%x, id: 0x%x\n",
                imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("ov16a10_mipi_raw: Read sensor id fail, id: 0x%x,sensor_id =0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	*(feature_data + 1) = imgsensor_info.min_shutter;
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = -17828000;
		break;
	#endif
