        ...
        do {
            *sensor_id = return_sensor_id()+2;
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                #endif
                pr_info("gc02m0b_mipi_mono2: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            pr_info("gc02m0b_mipi_mono2: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()+2;
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_info("gc02m0b_mipi_mono2: i2c mono2 write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            pr_info("gc02m0b_mipi_mono2: Read mono2 sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
