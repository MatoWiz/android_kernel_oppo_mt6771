		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("gc8054_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				#ifdef OPLUS_FEATURE_CAMERA_COMMON
				imgsensor_info.module_id = read_module_id();
				read_eeprom_CamInfo();
				read_eeprom_SN();
				pr_info("gc8054_mipi_raw: RENM0_module_id=%d\n",imgsensor_info.module_id);
			...
			pr_info("gc8054_mipi_raw: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
