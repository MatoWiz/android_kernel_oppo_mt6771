		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("imx616q2r_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				#ifdef VENDOR_EDIT
				imgsensor_info.module_id = read_module_id();
				read_eeprom_SN();
				read_eeprom_CamInfo();
				pr_info("imx616q2r_mipi_raw: IMX616Q2R_module_id=%d\n",imgsensor_info.module_id);
			...
			pr_info("imx616q2r_mipi_raw: Read sensor id fail, id: 0x%x\n",
				imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
