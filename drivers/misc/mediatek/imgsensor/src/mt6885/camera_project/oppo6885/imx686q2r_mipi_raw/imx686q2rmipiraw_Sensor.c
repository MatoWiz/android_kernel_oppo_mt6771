...
#define SENSOR_FUSION_THRESHOLD -17308750
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
            ...
            pr_info(
                "imx686q2r_mipi_raw: read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",
                read_cmos_sensor_8(0x0016),
                read_cmos_sensor_8(0x0017),
                read_cmos_sensor(0x0000));
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("imx686q2r_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n",
                    imgsensor.i2c_write_id, *sensor_id);
                #ifdef OPLUS_FEATURE_CAMERA_COMMON
                read_EepromQSC();
                read_eeprom_CamInfo();
                read_sensor_Cali();
                pr_info("imx686q2r_module_id=%d\n",imgsensor_info.module_id);
                #endif
                return ERROR_NONE;
            }
            pr_info("imx686q2r_mipi_raw: Read sensor id fail, id: 0x%x\n",
                imgsensor.i2c_write_id);
            retry--;
        } while (retry > 0);
        i++;
