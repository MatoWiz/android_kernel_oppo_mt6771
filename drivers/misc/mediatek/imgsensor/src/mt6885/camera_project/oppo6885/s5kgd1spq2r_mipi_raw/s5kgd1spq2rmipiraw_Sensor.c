...
#define SENSOR_FUSION_THRESHOLD -37320000
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			pr_info("s5kgd1spq2r_mipi_raw: read out sensor id 0x%x \n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("s5kgd1spq2r_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				#ifdef VENDOR_EDIT
				imgsensor_info.module_id = read_module_id();
				pr_info("s5kgd1spq2r_mipi_raw: s5kgd1spq2r_module_id=%d\n",imgsensor_info.module_id);
			...
			pr_info("s5kgd1spq2r_mipi_raw: Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
