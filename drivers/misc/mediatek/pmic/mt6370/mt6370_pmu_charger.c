...
#ifdef CONFIG_OPLUS_CHARGER_MTK6771
static int mt6370_enable_ship_mode(struct charger_device *chg_dev)
{
	int ret;
	...
	mutex_lock(&chg_data->adc_access_lock);
	dev_info(chg_data->dev, "%s \n", __func__);
	ret = mt6370_pmu_reg_write(chg_data->chip, MT6370_PMU_REG_RSTPASCODE1, 0xA9);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "set passcode1 fail\n");
		return ret;
	}
 	ret = mt6370_pmu_reg_write(chg_data->chip, MT6370_PMU_REG_RSTPASCODE2, 0x96);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "set passcode2 fail\n");
		return ret;
	}
	/* reset all chg/fled/ldo/rgb/bl/db reg and logic */
	ret = mt6370_pmu_reg_write(chg_data->chip, MT6370_PMU_REG_CORECTRL2, 0x7F);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "set reset bits fail\n");
		return ret;
	}
	/* disable chg auto sensing */
	mt6370_enable_hidden_mode(chg_data, true);
	ret = mt6370_pmu_reg_clr_bit(chg_data->chip, MT6370_PMU_REG_CHGHIDDENCTRL15, 0x01);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "set auto sensing disable\n");
		return ret;
	}
	mt6370_enable_hidden_mode(chg_data, false);
	mdelay(50);
	/* set shipping mode delay*/
	ret = mt6370_pmu_reg_set_bit(chg_data->chip,
		MT6370_PMU_REG_CHGCTRL2, MT6370_MASK_SHIP_MODE_DELAY);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "set shipping mode delay\n");
		return ret;
	}
	/* enter shipping mode */
	ret = mt6370_pmu_reg_set_bit(chg_data->chip, MT6370_PMU_REG_CHGCTRL2, 0x80);
	if (ret < 0) {
		dev_notice(&chg_dev->dev, "enter shipping mode\n");
		return ret;
	}
